...
const AccountForClient = () => {
  ...
  const getFavourites = async () => {
    try {
      const response = await fetch(`${BACKEND_URL}/api/user/favourite/get`, {
        credentials: "include",
      });
          ...
          <div className="c-account-main">
            {userDetails?.orders?.length ? (
              orders.map((value, index) => (
                <div
                  className="c-order-details-element"
                  key={index}
                  onClick={() => {
                    navigate(`/user/orders/${value._id}`, {
                      ...
                      <img
                        src={
                          userDetails?.contentCreator
                            ? `${value?.buyer?.profilePic}`
                            : `${value?.influencer?.profilePic}`
                        }
                        alt={value?.buyer?.name}
                      />
                    </div>
                    <div>
                      <h4>
            ...
            {userDetails?.favourites?.length > 0 ? (
              favourites.map((favourites, index) => (
                <div
                  ...
                  key={index}
                  onClick={() =>
                    navigate(`/influencer/${favourites.uniqueID}`, {
                      state: { uniqueID: favourites.uniqueID },
                    })
                  }
                >
                  <div className="order-item-info">
              ...
              <>
                ...
                <p style={{ fontSize: "13px", opacity: 0.7 }}>
                  Once you select favourite influencer data will show.
                </p>
              </>
            )}
          </div>
